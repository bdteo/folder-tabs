...
Set `texture="paper"` when the stack should read as real paper/cardstock. The texture is generated with portable CSS gradients, so it tiles cleanly out of the box, and it applies to the binder layers, folder sheets, content surface, and attached tab handles. Standalone `FolderTabs`, `FolderBinder`, `Folder`, and `FolderTabPanelStack` accept the same `texture` prop. This registry seed also includes image-backed paper assets and exports `getFolderPaperTextureStyle()`, `folderPaperTexturePresets`, and `folderPaperTexturePresetOptions` from `paperTextures.ts` when you want a stronger material texture. Bind the returned style object to the component root alongside `texture="paper"`.
